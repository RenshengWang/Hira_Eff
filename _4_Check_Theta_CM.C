...
*/
  ...
  */
  ...
  sprintf(NameTem,"./ESpec/f1_ESpec%s_%s_%s_%s_b_%.1f_%.1f.root",EffCor_Tag.c_str(),SystemTag.c_str(),RunTag.c_str(),Hira_BadMap_Version.c_str(),ImpactPar_Range[0],ImpactPar_Range[1]);
  TFile* f1_ESpec = new TFile(NameTem,"update");
  for(int i=0;i<ThetaNum;i++)
  {
    h2_Theta_Ekin_CM[2]->Write("",TObject::kOverwrite);
    h1_ESpec_T[i]->Write("",TObject::kOverwrite);
    h2_Theta_Ekin_CM[3]->Write("",TObject::kOverwrite);
