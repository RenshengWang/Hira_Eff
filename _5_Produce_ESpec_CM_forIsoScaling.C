...
*/
  ...
  {
    sprintf(NameTem,"h2_%s_b_%.1f_%.1f_Theta_Ekin_CM_GeoReactionEff_ImpactPars",ParticleName[i].c_str(),ImpactPar_Range[0],ImpactPar_Range[1]);
    h2_Theta_Ekin_CM_GeoReactionEff[i] = (TH2D*) f1_ESpec->Get(NameTem);
    
    sprintf(NameTem,"h2_%s_b_%.1f_%.1f_Theta_Ekin_CM_GeoEff_ImpactPars",ParticleName[i].c_str(),ImpactPar_Range[0],ImpactPar_Range[1]);
    h2_Theta_Ekin_CM_GeoEff[i] = (TH2D*) f1_ESpec->Get(NameTem);

    sprintf(NameTem,"h2_%s_b_%.1f_%.1f_Theta_Ekin_CM_noEff_ImpactPars",ParticleName[i].c_str(),ImpactPar_Range[0],ImpactPar_Range[1]);
